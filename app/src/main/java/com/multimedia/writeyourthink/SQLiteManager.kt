...
class SQLiteManager(context: Context?, name: String?, factory: CursorFactory?, version: Int) :
    ...
    fun insertUser(userUID: String, userName: String, userProfile: String, userEmail: String) {
        val db = writableDatabase
        val query =
            "INSERT INTO $tableNameUser VALUES(null,'$userUID', '$userName', '$userProfile', '$userEmail');"
        db.execSQL(query)
    }
    ...
    fun updateUser(
        ...
        val db = writableDatabase
        val query =
            "UPDATE $tableNameUser SET userUID='$userUID', userName='$userName', userProfile='$userProfile', userEmail='$userEmail' WHERE id=$id;"
        db.execSQL(query)
    }
